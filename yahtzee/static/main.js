...
function startRoll() {
  if (!getFromLocalStorageObject("clientRoundState", "currentRollCount")) {
    ...
    getFromLocalStorageObject("clientRoundState", "currentRollCount") < 4
  ) {
    rollMeatAndPotatoes();
  }
}
...
function stopRoll() {
  if (getFromLocalStorageObject("clientRoundState", "currentRollCount") === 1) {
    ...
    getFromLocalStorageObject("clientRoundState", "currentRollCount") === 2
  ) {
    // Increment roll count, stop rolling animation, allow user to select/unselect images
...
for (let l = 0, l < someArray.length; l++) {
  const element = someArray[l];
